...
export interface System {id:string;name:string;color:string;description:string}
...
export interface Part {id:string;name:string;conceptId:string;system:SystemId;chunk:number;positions:number;normals:number;indices:number;vertexCount:number;indexCount:number;bounds:[number[],number[]];explodeOffset?:[number,number,number]}
export interface Concept {id:string;name:string;elements:string[]}
export interface Atlas {version:string;itemName?:string;subtitle?:string;sex?:'male';source?:string;scope?:string;parts:Part[];concepts:Concept[];systems?:System[];explanations?:Record<string,string>;presets?:{name:string;systems:string[]}[];chunks:{url:string;bytes:number;gzip?:string;gzipBytes?:number}[];triangles:number}
export type View = 'three-quarter'|'front'|'back'|'side';
export interface SceneState {inspectorOpen?:boolean;explode:number;visible:SystemId[];selected:string[];isolate:boolean;view:View;rotate:boolean;reset:number;sound?:boolean}
...
export function explanation(name:string,system:SystemId,atlas?:Atlas|null){
 if(atlas?.explanations){
  const found=atlas.explanations[name.toLowerCase()];
  if(found)return found;
 }
 return EXPLANATIONS[name.toLowerCase()] ?? (atlas?.systems??SYSTEMS).find(s=>s.id===system)?.description ?? '';
}
